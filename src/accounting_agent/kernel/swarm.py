...
log = logging.getLogger("accounting_agent.kernel.swarm")
    ...
    def ensure_init(self, **ray_init_kwargs: Any) -> None:
        ...
        if not _has_ray():
            raise ImportError(
                ...
                "Install with: pip install 'accounting-agent-layer[ray]'"
            )
        import ray
...
def submit_skill_remote(skill_name: str, context: dict[str, Any]) -> str:
    ...
    from accounting_agent.kernel.registry import default_registry
