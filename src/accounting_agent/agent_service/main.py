...
from accounting_agent.agent_worker.celery_app import celery_app
from accounting_agent.common.db import db_session, make_engine
from accounting_agent.common.logging import configure_logging, get_logger
...
app = FastAPI(title="Accounting Agent Layer Service", version=os.getenv("APP_VERSION", "0.1.0"))
...
def _dispatch_run_local(run_id: str) -> None:
    def _runner() -> None:
        try:
            from accounting_agent.agent_worker.tasks import dispatch_run as worker_dispatch
    ...
    thread = threading.Thread(
        target=_runner,
        name=f"accounting_agent-local-run-{run_id[:8]}",
        daemon=True,
    )
    thread.start()
    ...
    if preferred_executor == "celery":
        try:
            celery_app.send_task(
                "accounting_agent.agent_worker.tasks.dispatch_run",
                args=[run_id],
                queue=queue_name,
            )
...
_ATTACH_UPLOAD_DIR = Path(os.getenv("AGENT_UPLOAD_DIR", "/tmp/accounting_agent_uploads"))
_ATTACH_ALLOWED_EXT = {
    ".pdf": "application/pdf",
    ".xml": "application/xml",
    ".jpg": "image/jpeg",
...
# ---------------------------------------------------------------------------
# Accounting endpoints  (ERP AI Kế toán)
# ---------------------------------------------------------------------------
...
def reprocess_voucher(
    ...
    restore_gzip_b64 = str(req.get("file_content_gzip_b64") or "").strip()
    ...
    if (attachment is None or not uri_ready) and (restore_b64 or restore_gzip_b64):
        try:
            if restore_gzip_b64:
                zipped_blob = base64.b64decode(restore_gzip_b64, validate=True)
                blob = gzip.decompress(zipped_blob)
            else:
                blob = base64.b64decode(restore_b64, validate=True)
        except Exception as exc:
            detail = "file_content_gzip_b64 không hợp lệ" if restore_gzip_b64 else "file_content_b64 không hợp lệ"
            raise HTTPException(status_code=422, detail=detail) from exc
        if not blob:
            detail = "file_content_gzip_b64 rỗng" if restore_gzip_b64 else "file_content_b64 rỗng"
    ...
    run_payload = {
        ...
        "restore_attachment_supplied": bool(restore_b64 or restore_gzip_b64),
    }
    run = AgentRun(
        run_id=new_uuid(),
...
def list_available_graphs() -> dict[str, Any]:
    ...
    try:
        from accounting_agent.graphs.registry import is_available, list_graphs
        return {
...
def ray_status() -> dict[str, Any]:
    ...
    readiness = _executor_readiness()
    try:
        from accounting_agent.kernel.swarm import get_swarm, is_available
...
def vn_feeder_status() -> dict[str, Any]:
    ...
    from accounting_agent.agent_service.vn_feeder_engine import (
        get_target_events_per_min as _feeder_target_epm,
    )
    from accounting_agent.agent_service.vn_feeder_engine import is_running as _feeder_is_running
...
def _report_export_dir() -> Path:
    path = Path(os.getenv("AGENT_REPORT_EXPORT_DIR", "/tmp/accounting_agent_reports"))
...
def _render_report_html(data: dict[str, Any]) -> str:
    ...
    return (
        "<html><head><meta charset='utf-8'><title>Accounting Agent Layer Report</title></head><body>"
