...
from accounting_agent.common.utils import (
...
from accounting_agent.flows.voucher_ingest import flow_voucher_ingest

# LangGraph integration — graceful degradation if not installed
try:
    from accounting_agent.graphs.registry import get_graph
    from accounting_agent.graphs.registry import is_available as _graphs_available
...
@celery_app.task(name="accounting_agent.agent_worker.tasks.dispatch_run", bind=True)
...
def _wf_voucher_classify(run_id: str) -> dict[str, Any]:
    ...
    from accounting_agent.flows.voucher_classify import flow_voucher_classify
...
def _wf_voucher_reprocess(run_id: str) -> dict[str, Any]:
    ...
    try:
        with db_session(engine) as s:
            ...
            attachment_id = str(raw_payload.get("attachment_id") or attachment_id or "").strip()
            if not attachment_id:
                raise RuntimeError("Voucher không có attachment_id hợp lệ để reprocess")
            raw_payload["status"] = "review"
            raw_payload["quality_status"] = "review"
