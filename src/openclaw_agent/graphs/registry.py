...
log = logging.getLogger("openclaw.graphs.registry")

# ---------------------------------------------------------------------------
# Lazy builders — each returns a compiled LangGraph when called
# ---------------------------------------------------------------------------
_GRAPH_BUILDERS: dict[str, str] = {
    "journal_suggestion": "openclaw_agent.graphs.journal_suggestion_graph:build_journal_suggestion_graph",
    "bank_reconcile": "openclaw_agent.graphs.bank_reconcile_graph:build_bank_reconcile_graph",
    "soft_checks": "openclaw_agent.graphs.soft_checks_graph:build_soft_checks_graph",
    "cashflow_forecast": "openclaw_agent.graphs.cashflow_forecast_graph:build_cashflow_forecast_graph",
    "tax_report": "openclaw_agent.graphs.tax_report_graph:build_tax_report_graph",
}
...
def get_graph(name: str) -> Any:
    ...
    if not _has_langgraph():
        raise ImportError("langgraph is not installed — install with: pip install 'openclaw-agent-erpx[graphs]'")
